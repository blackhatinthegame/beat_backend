...
router.post('/refresh', (req, res) => {
  ...
  try {
    const payload = jwt.verify(token, config.jwtRefreshSecret) as { sub: string; role: Role };
    const user = db.findUserById(payload.sub);
    if (!user || user.refreshToken !== token) throw new Error('invalid');
    const accessToken = signAccess(user.id, user.role);
    const refreshToken = signRefresh(user.id, user.role);
    db.saveRefreshToken(user.id, refreshToken);
