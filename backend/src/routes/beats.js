...
router.post('/create', requireAuth, requireRole('CREATOR', 'ADMIN'), upload.fields([
...
router.get('/list', (req, res) => {
    ...
    if (mood)
        results = results.filter(b => b.mood === mood);
    if (bpmMin)
        results = results.filter(b => b.bpm >= Number(bpmMin));
    if (bpmMax)
        results = results.filter(b => b.bpm <= Number(bpmMax));
    if (sort === 'price_asc')
...
router.get('/search', (req, res) => {
    return router.handle({ ...req, url: '/list', method: 'GET' }, res, () => { });
});
